...
use twenor_tools::input_file_dialog;
...
enum MethodName {
    GetRecordboxLibraryPath,
    // GetLibrary,
}

impl MethodName {
    fn from_str(s: &str) -> Option<MethodName> {
        match s {
            "getRecordboxLibraryPath" => Some(MethodName::GetRecordboxLibraryPath),
            // "getLibrary" => Some(MethodName::GetLibrary),
            _ => None,
        }
    }
}
...
pub fn init_listeners(a: &mut tauri::App, l: Arc<Mutex<Library>>) {
    ...
    listen(
        ...
        |library: &mut Library, app_handle: &AppHandle, data: Data| {
            ...
            LOG.debug(&format!("Data: {:?}", data));
            // let filters = data.unwrap_filters();
            // let title = data.unwrap_title();

            // let file_buf = input_file_dialog(&title, &filters);
            // LOG.debug(&format!("File buffer: {:?}", file_buf));
            // emitter::responde(app_handle, "RECEIVE_FILE_DIALOG", &file_buf);
        },
    );
...
//     match library.update_config(key, value) {
//         Ok(_) => {
//             emitter::config(app_handle, library);
//             match library.save() {
...
//         }
//         Err(err) => {
//             LOG.error(&format!("config not updated: {}", err));
//             emitter::error(app_handle, "config not updated");
//         }
//     }
// }
